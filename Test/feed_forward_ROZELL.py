...
dict_data = pandas.read_csv(dict_path)
Lca = r_retwork(dict_data.values)
...
pdb.set_trace()
sparse_data = np.zeros((len(training_set) , Lca.dictionary.shape[1]))
for i in range(len(training_set)):
    Lca.set_stimulus(training_set[i][0])
    Lca.generate_sparse()
    sparse_data[:,i] = Lca.a.copy()
